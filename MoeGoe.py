...
def speakout(inputText):
    modelPath =r'D:\DEEP LEARNING\CHATGPT\model\japanese_riddle_joker\365_epochs.pth'
    configPath = r'D:\DEEP LEARNING\CHATGPT\config\japanese_riddle_joker\config.json'
    inputText = inputText
    outputPath = r'D:\DEEP LEARNING\CHATGPT\result_audio\output.wav'
    selectSpeakId = 6
    iscontinue = 'n'
    ...
    hps_ms = ut.get_hparams_from_file(config)
    ...
    net_g_ms = SynthesizerTrn(
        ...
        **hps_ms.model)
    _ = net_g_ms.eval()
    ut.load_checkpoint(model, net_g_ms)

    def voice_conversion():
        audio_path = input('Path of an audio file to convert:\n')
        print_speakers(speakers)
        audio = ut.load_audio_to_torch(
